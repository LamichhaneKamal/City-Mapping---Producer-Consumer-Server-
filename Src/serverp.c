...
unsigned int timelapse, total_trans= 0;
...
/* Run the consumers */
 ...
 //fprintf(stdout, "    ******** Transactions processed ********\n | Past ten seconds | Total | Remaining time | \n");
 while(RTime > 0)
   {
...
fprintf(stdout, "System exited normally \n");
...
void produce(int pnum)
{
 ...
 /* const char base_str[] = "A string to be used as a base message. It is just a container for a very long string with up to 256 characters. Part of it will be sent from the producer to the consumer depending on the calculated length bu using the Binomial distribution.**************"; */
  ...
  /* Open the queue */
  if ((qd_queue = mq_open (QUEUE_NAME, O_WRONLY | O_CREAT, QUEUE_PERMISSIONS, &attr)) == -1) 
    { perror ("Producer: mq_open"); exit (1); }
  ...
  while(state == RUN_NORMALLY) /* Start producing requests */
    {
      ...
      /* Print out statistics for this request */
     // fprintf(stderr, "P, %2d, %8ld, %8ld, %3d, %ld.%ld \n", pnum, BlockTime, NextReq, MsgLen, ts_asend.tv_sec, ts_asend.tv_nsec);

      /* Wait to produce the next request */
      usleep(NextReq);
    }
...
void consume(int pnum)
{
  ...
  unsigned long int ProcTime, IdleTime;
  ...
  if ((qd_queue = mq_open (QUEUE_NAME, O_RDONLY)) == -1) 
    { perror ("Consumer: mq_open"); exit (1); }
  
  /* Wait here until signal arrives */
  while(state == WAIT_ON_BARRIER);

  /* Run normally and consume requests  */
  while(state == RUN_NORMALLY)
	...
	if( state == RUN_NORMALLY ) /*The syscall was interrupted while waiting on an empty queue and the system is still running */
      ...
      IdleTime = utimedif(ts_breceive, ts_areceive); /* Compute waiting time on reception */
      IdleTime = (IdleTime > 20) ? IdleTime : 0; /* If waiting time was less than 20 microseconds disregard it */
      printf("Received buffer data is %s \n", rbuf); 

      ReqSize = strlen(rbuf);
      kill(getppid(),SIGUSR2); /* Notify the parent that it has processed a request */

      /* Calculate if the request will require IO. */
      if ( gsl_ran_bernoulli(r, P_i) ) 
	 ProcTime = gsl_ran_exponential(r,C_t1); /* No IO required */
      else
	 ProcTime = gsl_ran_exponential(r,C_t2);/* IO is required */

      /* Print out statistics for this request */
     // fprintf(stderr, "C, %2d, %8ld, %8ld, %3d, %ld.%ld\n", pnum, IdleTime, ProcTime, ReqSize, ts_areceive.tv_sec, ts_areceive.tv_nsec);
      usleep(ProcTime);
...
void test_input(int argc, char* argv[])
{
  ...
  if((RTime < MIN_RUN_TIME) || (RTime > MAX_RUN_TIME))
    ...
    /*process and check the last argument */
    base_str = argv[10] ;
  //Command_google_map = strtol(argv[10], &Str, 10);
 /* if (argv[10] != Str)
    {
      printf("ERROR: Command expected for google map <add or search>. \n");
      abort();
    } */
...
void clean_up_fork_failed(int pcount_total)
{
 ...
 for(pcount = 0; pcount < PNum; pcount++)
   if (Prod[pcount] != 0)
     kill(Prod[pcount],SIGTERM);

  /* Kill the consumers */
 for(pcount = 0; pcount < CNum; pcount++)
   if (Cons[pcount] != 0)
     kill(Cons[pcount],SIGTERM);
